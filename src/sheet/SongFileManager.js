...
class SongFileManager extends SM.Sheet.SheetManager {
    ...
    constructor(sheetName, folderIdOrPath, mode) {
        super(sheetName);
        this.sheetName = sheetName;
        this.folderId = folderIdOrPath != undefined ? folderIdOrPath : this.Sheet.getRange("B1").getValue();
        if (!isValidDriveId(this.folderId)) {
            this.folder = getFolderFromPath(this.folderId);
    ...
    process({ update }) {
        ...
        const isValidAudioFile = (/** @type {GoogleAppsScript.Drive.File} */ file) => {
            return (
                !file.isTrashed() && isAudioMimeType(file.getMimeType()) && !this.ignoreFile.includes(file.getName())
            );
        };
        ...
        while (filesIterator.hasNext()) {
            ...
            const song = popAndRemove(
                allSong,
                (s) => s.fileId == file.getId() || (s.name == tempSong.name && s.singer == tempSong.singer)
            );
            console.log(
                song?.i,
                tempSong.fileId,
            ...
            if (song) {
                ...
                if (song.fileId != file.getId()) {
                    ...
                    if (row > 0) {
                        this.Sheet.getRange(
                            row + this.statusColumn.headerCell.row,
                            this.statusColumn.headerCell.col
                        ).setValue("ok");
                        this.Sheet.getRange(
                            row + this.linkUpColum.headerCell.row,
                            this.linkUpColum.headerCell.col
        ...
        for (const s of allSong) {
            ...
            try {
                ...
                while (fs.hasNext()) {
                    let _f = fs.next();
                    if (
                        _f.getName().startsWith(`${s.vsinger} - ${s.vname}   ${s.singer}`) &&
                        _f.getMimeType().includes("flac")
                    ) {
                        f = _f;
                        break;
                    }
    ...
    /** add song to sheet at last blank row   */
    ...
    /** get all songs from sheet
     *  @param {{allowNull: boolean}} allowNull allow null song
     *  @returns {SongInfo[]}*/
